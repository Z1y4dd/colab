...
def create_correlation_figure(data, correlations, correlation_type, significance_level, well_name):
    ...
    return fig
def create_well_correlation_scatter_plots(df, well_name, log_lab_pairs, corr_threshold=0.3):
    """
    Generate scatter plots for all log-lab pairs for a given well,
    split by positive/negative correlation.
    Individual plots go to imgs/scatter_plots/positive and imgs/scatter_plots/negative.
    Also creates two summary plots (all positive/all negative subplots) in imgs/,
    sorted by absolute correlation (|r|) descending.
    """
    ...
    # Store tuples: (log_var, lab_var, x, y, r, p_val, n, hui)
    positive_points = []
    negative_points = []
    plot_stats = []

    for log_var, lab_var in log_lab_pairs:
        # Only use rows for this well and drop NaNs
        data = df[[log_var, lab_var]][df['Well'] == well_name].dropna()
        if data.shape[0] < 3:
            continue

        r, p_val = stats.pearsonr(data[log_var], data[lab_var])
        n = len(data)
        hui = abs(r) * n  # Calculate HUI (for reference only)

        if r > 0:
            save_dir = positive_dir
            positive_points.append((log_var, lab_var, data[log_var], data[lab_var], r, p_val, n, hui))
        else:
            save_dir = negative_dir
            negative_points.append((log_var, lab_var, data[log_var], data[lab_var], r, p_val, n, hui))

        # Build filename
        fname = f"{sanitize_filename(log_var.replace('Log_', ''))}_vs_{sanitize_filename(lab_var.replace('Lab_', ''))}_r{abs(r):.2f}.png"
        save_path = save_dir / fname

        # Individual plot
        fig, ax = plt.subplots(figsize=(7, 5))
        ax.scatter(data[log_var], data[lab_var],
                   color='darkgreen' if r > 0 else 'darkred',
                   alpha=0.7, edgecolors='k', s=45)
        if n > 3:
            z = np.polyfit(data[log_var], data[lab_var], 1)
            p = np.poly1d(z)
            x_line = np.linspace(data[log_var].min(), data[log_var].max(), 100)
            ax.plot(x_line, p(x_line),
                    color='darkgreen' if r > 0 else 'darkred',
                    linewidth=2.5,
                    label=f"r={r:.2f}  p={p_val:.1e}")
        ...
        ax.set_title(f"{log_var.replace('Log_', '')} vs {lab_var.replace('Lab_', '')}\n"
                     f"{well_name} | r={r:.2f}, n={n}, HUI={hui:.1f}", fontsize=10)
        ax.legend(loc='best', fontsize=9)
        ax.grid(True, linestyle=':', alpha=0.4)
        ax.set_facecolor('#e8f5e9' if r > 0 else '#ffebee')
        for spine in ax.spines.values():
            spine.set_edgecolor('darkgreen' if r > 0 else 'darkred')
            spine.set_alpha(0.5)
        ...
        plot_stats.append({
            'log_var': log_var, 'lab_var': lab_var, 'r': r, 'p_val': p_val, 'n': n, 'hui': hui, 'file': str(save_path)
        })

    # --- SORT by |correlation| descending ---
    positive_points = sorted(positive_points, key=lambda x: abs(x[4]), reverse=True)
    negative_points = sorted(negative_points, key=lambda x: abs(x[4]), reverse=True)
    plot_stats = sorted(plot_stats, key=lambda x: abs(x['r']), reverse=True)

    # --- Summary plot for all positive pairs (subplot grid) ---
    if positive_points:
        n = len(positive_points)
        ncols = 3
        nrows = math.ceil(n / ncols)
        fig, axes = plt.subplots(nrows, ncols, figsize=(5*ncols, 4*nrows))
        axes = np.array(axes).flatten()
        for idx, (log_var, lab_var, x, y, r, p_val, npts, hui) in enumerate(positive_points):
            ax = axes[idx]
            ax.scatter(x, y, color='darkgreen', alpha=0.5, s=30)
            if len(x) > 3:
                z = np.polyfit(x, y, 1)
                p = np.poly1d(z)
                x_line = np.linspace(x.min(), x.max(), 100)
                ax.plot(x_line, p(x_line), color='darkgreen', linewidth=2)
            ax.set_title(f"{log_var.replace('Log_', '')} vs {lab_var.replace('Lab_', '')}\n"
                         f"r={r:.2f}, n={npts}, HUI={hui:.1f}", fontsize=9)
            ax.set_facecolor('#e8f5e9')
            ax.grid(True, linestyle=':', alpha=0.3)
        for idx in range(n, len(axes)):
            axes[idx].set_visible(False)
        plt.tight_layout()
        plt.savefig(f"imgs/{well_name}_all_positive_points.png", dpi=150)
        plt.close()

    # --- Summary plot for all negative pairs (subplot grid) ---
    if negative_points:
        n = len(negative_points)
        ncols = 3
        nrows = math.ceil(n / ncols)
        fig, axes = plt.subplots(nrows, ncols, figsize=(5*ncols, 4*nrows))
        axes = np.array(axes).flatten()
        for idx, (log_var, lab_var, x, y, r, p_val, npts, hui) in enumerate(negative_points):
            ax = axes[idx]
            ax.scatter(x, y, color='darkred', alpha=0.5, s=30)
            if len(x) > 3:
                z = np.polyfit(x, y, 1)
                p = np.poly1d(z)
                x_line = np.linspace(x.min(), x.max(), 100)
        ...
        for idx in range(n, len(axes)):
            axes[idx].set_visible(False)
        plt.tight_layout()
        plt.savefig(f"imgs/{well_name}_all_negative_points.png", dpi=150)
        plt.close()

    print("\n" + "=" * 75)
    print(f"SCATTER PLOTS SUMMARY for {well_name}")
    print("=" * 75)
    print(f"Total pairs processed: {len(log_lab_pairs)}")
    print(f"Positive plots: {len(positive_points)}  |  Negative plots: {len(negative_points)}")
    print(f"Individual plots in: imgs/scatter_plots/positive and imgs/scatter_plots/negative")
    print(f"Summary plots: imgs/{well_name}_all_positive_points.png and imgs/{well_name}_all_negative_points.png")
    print("=" * 75 + "\n")

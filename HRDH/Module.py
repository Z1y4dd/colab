...
def create_comprehensive_correlation_scatter_plots(df_all, correlations_by_well_count, max_plots_per_figure=20):
    ...
    def create_individual_plot(pair, wells_data, info, correlation_type, save_path):
        ...
        title_line2 = f"Wells: {wells_str} | Avg |r| = {info['avg_abs_corr']:.3f}"
    ...
    if all_positive_corrs:
        ...
        for idx, (pair, wells_data, info) in enumerate(all_positive_corrs):
            ...
            filename = f"{idx+1:03d}_{log_name}_vs_{lab_name}_r{info['avg_abs_corr']:.2f}.png"
            save_path = positive_dir / filename
            
            # Create individual plot
            create_individual_plot(pair, wells_data, info, 'positive', save_path)
    ...
    if all_negative_corrs:
        ...
        for idx, (pair, wells_data, info) in enumerate(all_negative_corrs):
            ...
            # Create individual plot
            create_individual_plot(pair, wells_data, info, 'negative', save_path)
    ...
    for i, (pair, wells_data, info) in enumerate(all_positive_corrs[:10]):
        log_var, lab_var = pair
        wells_involved = [well.replace('HRDH_', '') for well, _ in wells_data]
        print(f"{i+1}. {log_var.replace('Log_', '')} vs {lab_var.replace('Lab_', '')}: " +
              f"|r̄| = {info['avg_abs_corr']:.3f} (Wells: {', '.join(wells_involved)})")
    
    print("\nTop 10 Negative Correlations:")
    for i, (pair, wells_data, info) in enumerate(all_negative_corrs[:10]):
        log_var, lab_var = pair
        wells_involved = [well.replace('HRDH_', '') for well, _ in wells_data]
        print(f"{i+1}. {log_var.replace('Log_', '')} vs {lab_var.replace('Lab_', '')}: " +
              f"|r̄| = {info['avg_abs_corr']:.3f} (Wells: {', '.join(wells_involved)})")
...
def create_combined_correlation_heatmap(df_all, lab_columns, log_columns, well_correlations):
    ...
    for log_var in log_columns:
        for lab_var in lab_columns:
            # Get all valid data points across all wells
            valid_mask = (~df_all[log_var].isna()) & (~df_all[lab_var].isna())
            valid_data = df_all.loc[valid_mask, [log_var, lab_var]]
            
            if len(valid_data) >= 10:  # Minimum sample size for reliable correlation
                corr_value, p_value = stats.pearsonr(valid_data[log_var], valid_data[lab_var])
                combined_corr_matrix.loc[log_var, lab_var] = corr_value
                sample_size_matrix.loc[log_var, lab_var] = len(valid_data)
            else:
    ...
    for i in range(len(combined_corr_matrix.index)):
        for j in range(len(combined_corr_matrix.columns)):
            if not pd.isna(combined_corr_matrix.iloc[i, j]):
                r_val = combined_corr_matrix.iloc[i, j]
                n_samples = sample_size_matrix.iloc[i, j]
                # Use clear formatting for sample size
    ...
    for i in range(len(combined_corr_matrix.index)):
        for j in range(len(combined_corr_matrix.columns)):
            if not mask_weak.iloc[i, j] and not pd.isna(combined_corr_matrix.iloc[i, j]):
                r_val = combined_corr_matrix.iloc[i, j]
                n_samples = sample_size_matrix.iloc[i, j]
                # Clean formatting - just show correlation and sample size
    ...
    for threshold, label in [(0.7, "Very Strong"), (0.5, "Strong"), (0.3, "Moderate")]:
        ...
        print(f"{label} (|r| ≥ {threshold}): {count} pairs ({count/total_valid*100:.1f}%)")
...
def create_improved_correlation_heatmaps(well_correlations, correlations_by_well_count, df_all, lab_columns, log_columns):
    ...
    # --- Figure 1: Average Correlations with Well Count ---
    fig1 = plt.figure(figsize=(16, 12))
    ax1 = fig1.add_subplot(111)
    
    # Create custom annotation with smaller font for better fit
    ...
    sns.heatmap(avg_corr_matrix.astype(float), 
                ...
                annot_kws={'size': 8, 'va': 'center'},  # Smaller font size
                cmap='RdYlGn', 
                center=0, vmin=-1, vmax=1,
                cbar_kws={'label': 'Average Correlation (r)', 'shrink': 0.8, 'pad': 0.02},
                linewidths=0.5,
                linecolor='gray',
                square=True,  # Square cells for better text fit
                ax=ax1)
    
    ax1.set_title('Average Correlations Across Wells\n(value shows r and number of wells)', 
                  fontsize=18, fontweight='bold', pad=20)
    ax1.set_xlabel('Laboratory Measurements', fontsize=14, fontweight='bold', labelpad=10)
    ax1.set_ylabel('Geophysical Log Measurements', fontsize=14, fontweight='bold', labelpad=10)
    ...
    plt.savefig('imgs/heatmap_1_average_correlations.png', dpi=300, bbox_inches='tight', pad_inches=0.3)
    plt.show()
